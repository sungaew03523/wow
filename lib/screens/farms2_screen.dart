...
class Farm2 {
  ...
  Farm2({
    ...
    this.resSavings = 0.0, // Доп. бонус к экономии (например 0.5 для +50%)
    this.multiYield = 0.5,
    this.iconUrl,
  ...
  factory Farm2.fromFirestore(DocumentSnapshot doc) {
    ...
    return Farm2(
      ...
      resSavings: (data['resSavings'] ?? 0.0).toDouble(), // Default bonus 0%
      multiYield: (data['multiYield'] ?? 0.5).toDouble(),
...
class _Farms2ScreenState extends State<Farms2Screen> {
              ...
              onPressed: () async {
                final bool? confirm = await showDialog<bool>(
                  ...
                  builder: (context) => AlertDialog(
                    ...
                    content: const Text(
                        'Вы уверены, что хотите сбросить все ручные правки цен?'),
                    actions: [
                      TextButton(
          ...
          return StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              final farms = snapshot.data!.docs
                  .map((doc) => Farm2.fromFirestore(doc))
                  .toList();
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('Фармов пока нет. Создайте первый!',
                          style: TextStyle(color: Colors.grey)),
                      const SizedBox(height: 24),
                      ElevatedButton(
                          onPressed: () => _showFarmDialog(),
                          child: const Text('Создать Фарм')),
                    ],
                  ),
                );
              }
  ...
  List<RecipeComponent> _parseFormula(
      String formula, Map<String, AuctionItem> favorites) {
  ...
  String _generateFormula(List<RecipeComponent> components) {
    ...
    return components.map((c) {
      if (c.options.length > 1) {
        ...
        String result =
            '${c.options.last.quantity}*"${c.options.last.itemName}"';
  ...
  Future<void> _showFarmDialog({Farm2? farm}) async {
    ...
    final Map<String, AuctionItem> favoritesMap = {
      for (var doc in favoritesSnapshot.docs)
        doc.id: AuctionItem.fromFirestore(doc)
    };
    ...
    // Списки компонентов
    List<RecipeComponent> costs =
        _parseFormula(farm?.costsFormula ?? '', favoritesMap);
    List<RecipeComponent> revenue =
        _parseFormula(farm?.revenueFormula ?? '', favoritesMap);

    await showDialog(
      context: context,
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) {
          Widget buildPickerTile(AuctionItem item, StateSetter dialogState,
              List<RecipeComponent> list,
              [RecipeComponent? parent]) {
            return ListTile(
              leading: item.iconUrl != null
                  ? Image.network(item.iconUrl!, width: 24)
                  : null,
              title: Text(item.name),
              onTap: () {
                dialogState(() {
                  if (parent != null) {
                    final baseQuantity = parent.options.isNotEmpty
                        ? parent.options.first.quantity
                        : '1';
                    parent.options.add(RecipeItem(
                        itemName: item.name,
                        iconUrl: item.iconUrl ?? '',
                        quantity: baseQuantity));
                  } else {
                    list.add(RecipeComponent(options: [
                      RecipeItem(
                          itemName: item.name, iconUrl: item.iconUrl ?? '')
                    ]));
                  }
                });
          ...
          void showPicker(List<RecipeComponent> list,
              [RecipeComponent? parent]) {
            final matchingItems = favoritesMap.values
                .where((item) => item.professions.contains(selectedProfession))
                .toList();
            final otherItems = favoritesMap.values
                .where((item) => !item.professions.contains(selectedProfession))
                .toList();
                  ...
                  child: ListView(
                    ...
                    children: [
                      if (matchingItems.isNotEmpty) ...[
                        ...
                        ...matchingItems.map((item) => buildPickerTile(
                            item, setDialogState, list, parent)),
                        const Divider(),
                      ],
                      const Padding(
                      ...
                      ...otherItems.map((item) =>
                          buildPickerTile(item, setDialogState, list, parent)),
                    ],
                  ),
                ),
              ),
          ...
          Widget buildComponentRow(
              RecipeComponent comp, List<RecipeComponent> list) {
            return Column(
                ...
                child: Column(
                  ...
                  children: [
                    TextField(
                        ...
                        decoration: const InputDecoration(
                            labelText: 'Название группы/фарма')),
                    TextField(
                        controller: itemNameController,
                        decoration: const InputDecoration(
                            labelText: 'Название крафта (предмета)')),
                    Row(
                      children: [
                        Expanded(
                          child: DropdownButtonFormField<String>(
                            ...
                            items: _professions
                                ...
                                .toList(),
                            onChanged: (v) =>
                                setDialogState(() => selectedProfession = v),
                            decoration:
                                const InputDecoration(labelText: 'Профессия'),
                          ),
                        ),
                    ...
                    const SizedBox(height: 16),
                    Text('Затраты (Реагенты)',
                        style: GoogleFonts.oswald(
                            color: Colors.redAccent, fontSize: 16)),
                    ...costs.map((c) => buildComponentRow(c, costs)),
                    TextButton.icon(
                        onPressed: () => showPicker(costs),
                        icon: const Icon(Icons.add, size: 16),
                        label: const Text('Добавить затрату')),
                    const SizedBox(height: 16),
                    Text('Выручка (Что получим)',
                        style: GoogleFonts.oswald(
                            color: Colors.blueAccent, fontSize: 16)),
                    ...revenue.map((c) => buildComponentRow(c, revenue)),
                    TextButton.icon(
                        onPressed: () => showPicker(revenue),
                        icon: const Icon(Icons.add, size: 16),
                    ...
                    Slider(
                        ...
                        max: 100,
                        divisions: 100,
                        onChanged: (v) => setDialogState(() => resValue = v)),
                    Slider(
                        value: resSavings,
                        min: 0,
                        max: 1.0,
                        divisions: 100,
                        onChanged: (v) => setDialogState(() => resSavings = v)),
                    const SizedBox(height: 8),
                    Text(
                        'Мультикрафт: ${multiValue.toInt()}% (Бонус: ${(multiYieldVal * 100).toInt()}%)',
                        style: const TextStyle(fontSize: 12)),
                    Slider(
                        value: multiValue,
                        min: 0,
                        max: 100,
                        divisions: 100,
                        onChanged: (v) => setDialogState(() => multiValue = v)),
                    Slider(
                        value: multiYieldVal,
                        min: 0,
                        max: 2.0,
                        divisions: 200,
                        onChanged: (v) =>
                            setDialogState(() => multiYieldVal = v)),
                  ],
                ),
              ),
            ),
  ...
  Future<void> _duplicateFarm(Farm2 farm) async {
    try {
      ...
      final String originalName =
          farm.name.isNotEmpty ? farm.name : farm.itemName;
...
class Farm2Card extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final double baseCost =
        _calculateFormula(farm.costsFormula, favorites, overrides);
            ...
            children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                                    ...
                                    onPressed: () async {
                                      final bool? confirm =
                                        ...
                                        builder: (context) => AlertDialog(
                                          ...
                                          content: const Text(
                                              'Вы уверены, что хотите удалить этот фарм?'),
                                          actions: [
                                            TextButton(
                        ...
                        Row(
                          children: [
                            Text(farm.profession,
                                style: const TextStyle(
                                    color: Colors.white60, fontSize: 13)),
                            if (farm.name.isNotEmpty &&
                                farm.itemName.isNotEmpty)
                              Text(' • ${farm.name}',
                                  style: const TextStyle(
                              ...
                              child: InkWell(
                                ...
                                child: const Padding(
                                  ...
                                  child: Icon(Icons.calculate_outlined,
                                      size: 16, color: Color(0xFFD4BF7A)),
                                ),
                              ),
                            ),
                          ],
              ...
              Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _buildStatBadge(Icons.auto_fix_high,
                          '${farm.resourcefulness.toInt()}%'),
                      const SizedBox(width: 6),
                      _buildStatBadge(
                          Icons.layers, '${farm.multicraft.toInt()}%'),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildStatBadge(IconData icon, String value) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(color: const Color(0xFFD4BF7A), width: 0.5)),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
  ...
  void _showCalculationDetails(BuildContext context,
      Map<String, AuctionItem> favorites, Map<String, double> overrides) {
    ...
    for (var name in costItems) {
      double p =
          overrides[name] ?? favorites[name]?.weightedAveragePrice ?? 0.0;
    ...
    if (farm.costsFormula.isNotEmpty) {
      breakdownWidgets.add(const Text('СТОИМОСТЬ КАЖДОГО РЕАГЕНТА:',
          style: TextStyle(color: Colors.grey, fontSize: 10, fontWeight: FontWeight.bold)));
      breakdownWidgets.add(const SizedBox(height: 4));
      ...
      for (var part in parts) {
        ...
        if (nameMatches.isNotEmpty) {
           partTitle = nameMatches.map((m) => m.group(1)).join(' / ');
        }
        
        breakdownWidgets.add(Padding(
          padding: const EdgeInsets.only(bottom: 6.0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(partTitle, style: const TextStyle(color: Colors.white70, fontSize: 11), maxLines: 2, overflow: TextOverflow.ellipsis),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
      ...
      breakdownWidgets.add(const Divider(color: Colors.white10, height: 16));
    }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Детали расчета',
            style: TextStyle(color: Color(0xFFD4BF7A))),
        content: SizedBox(
          width: double.maxFinite,
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text('ДОХОД (с учётом -5% комиссии):',
                    style: TextStyle(
                ...
                _buildDebugRow(
                    'Шанс Находчивости', '${farm.resourcefulness.toInt()}%'),
                _buildDebugRow('Множитель Находчивости',
                    'x${resGainMultiplier.toStringAsFixed(3)}'),
                const SizedBox(height: 4),
                _buildDebugRow('Шанс Мультикрафта', '${farm.multicraft.toInt()}%'),
                _buildDebugRow('Множитель Мультикрафта',
                    'x${(1 + (farm.multicraft / 100 * farm.multiYield)).toStringAsFixed(3)}'),
                const Divider(color: Colors.white10, height: 24),
                
                ...breakdownWidgets,
                
                const Text('ИТОГОВАЯ МАТЕМАТИКА (за 1 крафт):',
                    style: TextStyle(
                        color: Colors.grey,
                        fontSize: 10,
                        fontWeight: FontWeight.bold)),
  ...
  double _calculateFormula(String formula, Map<String, AuctionItem> favorites,
    ...
    while (true) {
      ...
      workingFormula =
          workingFormula.replaceFirst(match.group(0)!, minValue.toString());
    }
    ...
    String safeFormula =
        workingFormula.replaceAll(RegExp(r'[^\d.+\-*/() ]'), '');
  ...
  double _calculateFormulaWithResourcefulness(
      String formula,
      Map<String, AuctionItem> favorites,
    ...
    // 4. После вычисления min, формула должна состоять только из сложений.
    // Пример: 3*599.5 + 1*10 + 40
    // Мы делим по плюсу, считаем каждое слагаемое, применяем скидку Находчивости
    // ко всему слагаемому отдельно
    
    double totalCostWithResourcefulness = 0.0;
    final parts = workingFormula.split('+');
    
    for (var part in parts) {
       String safePart = part.replaceAll(RegExp(r'[^\d.+\-*/() ]'), '');
       if (safePart.trim().isNotEmpty) {
           try {
               double partCost = safePart.interpret().toDouble();
               totalCostWithResourcefulness += partCost * resMultiplier;
           } catch (e) {
               debugPrint('Ошибка вычисления части формулы ($safePart): $e');
           }
       }
    }

    return totalCostWithResourcefulness;
  }
      ...
      children: [
        ...
        Wrap(
          ...
          children: allItems.map((name) {
            ...
            final double price = isOverridden
                ? overrides[name]!
                : (item?.weightedAveragePrice ?? 0.0);
            ...
            return InkWell(
                ...
                final double? result = await showDialog<double>(
                  ...
                  builder: (context) => AlertDialog(
                    ...
                    content: TextField(
                      ...
                      decoration: const InputDecoration(
                          labelText: 'Цена за 1 шт. (золото)', suffixText: 'з'),
                      keyboardType:
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color:
                      isOverridden ? Colors.blue.withAlpha(40) : Colors.black26,
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                      color: isOverridden ? Colors.blueAccent : Colors.white10,
                      width: 1),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    ...
                    const SizedBox(width: 4),
                    Text('${price.toStringAsFixed(0)}з',
                        style: TextStyle(
                            fontSize: 11,
                            color: isOverridden
